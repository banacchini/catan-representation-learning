"""
Generator v2 zbioru Catan — per-karta, JEDNA perspektywa na grę.

Produkuje DWIE powiązane tabele parquet (w chunkach):
  timesteps_*.parquet    - pełna sekwencja per akcja (wejście dla VAE-LSTM/RSSM).
                           Zawiera WSZYSTKIE kroki (też bez kart na ręce).
  card_samples_*.parquet - jedna próbka na POJEDYNCZĄ kartę trzymaną w danym
                           kroku (5-klasowa klasyfikacja typu). Łączy się z
                           timesteps po (game_id, observed_color, action_index).

Decyzje:
  - DOKŁADNIE JEDNA perspektywa na grę. Jeśli przy stole jest kilku silnych
    botów, losowo wybierany jest jeden jako obserwowany. Jeśli przy stole NIE
    ma żadnego silnego bota — gra jest pomijana (i nie jest nawet rozgrywana).
    Eliminuje to korelację między perspektywami tej samej gry.
...
Perspektywa = neutralny obserwator stołu: tylko informacja PUBLICZNA.
Maskowane (ukryte): rozbicie kart dev obserwowanego (target), rozbicie zasobów
każdego gracza, ACTUAL_VICTORY_POINTS, skład talii dev (tylko jej długość jawna).

GWARANCJA POPRAWNOŚCI: w każdym kroku śledzona ręka MUSI zgadzać się z IN_HAND
z player_state (assert w kodzie).

Uruchom:
    uv run generate_dataset_v2.py --num 10000 --out-dir data --chunk-size 1000
"""
...
def production_access(state, color):
    """JAWNA cecha: ważony p-stwem kości dostęp do produkcji każdego zasobu.
    Sumuje node_production po budynkach gracza; miasto liczy podwójnie.
    node_production[node_id] = Counter{resource: p-stwo}."""
    prod = Counter()
    node_prod = state.board.map.node_production
    for node_id, bld in state.board.buildings.items():
        if bld[0] != color:
            continue
        mult = 2 if bld[1] == "CITY" else 1
        for res, p in node_prod.get(node_id, {}).items():
            prod[res] += mult * p
    return prod


class Snapshotter(GameAccumulator):
    """Lekki snapshot stanu PRZED każdą akcją (raz na akcję)."""
    ...
    def step(self, game_before_action, action):
        ...
        snap = {
            ...
            "ps": {k: ps[k] for k in ps},
            "robber_block": {COLORS[i]: robber_blocks(st, COLORS[i]) for i in range(4)},
            "color_to_index": dict(st.color_to_index),
            "production": {COLORS[i]: production_access(st, COLORS[i]) for i in range(4)},
            "bank": list(st.resource_freqdeck),          # [WOOD,BRICK,SHEEP,WHEAT,ORE]
            "dev_deck_remaining": len(st.development_listdeck),  # tylko LICZBA (jawna)
        }
        self.snaps.append(snap)


def player_public_features(ps, pi, is_current, rel_pos, prod):
    """Stan gracza o P-indeksie `pi`, nazwany wg pozycji względnej `rel_pos`
    (p0=obserwowany, p1..p3=przeciwnicy). `prod` = produkcja tego gracza."""
    pre = f"P{pi}_"
    rp = rel_pos
    out = {
    ...
    # produkcja per zasób (jawna, ważona p-stwem kości)
    for r in RESOURCES:
        out[f"p{rp}_prod_{r.lower()}"] = round(prod.get(r, 0.0), 4)
    return out


def build_perspective(snaps, observed_color, game_id, table_str,
                      observed_type, winner_type):
    """Buduje timesteps + card_samples dla JEDNEJ perspektywy (observed_color)."""
    ts_rows, card_rows = [], []
    hand = []
    last_buy = None
    last_play = None
    ...
    for idx, snap in enumerate(snaps):
        ...
        for rel_pos, color in enumerate(order):
            pi = c2i[color]
            prod = snap["production"].get(color, {})
            feats.update(player_public_features(ps, pi, color == cur, rel_pos, prod))
        # akcja
        ...
        for ri, r in enumerate(RESOURCES):
            feats[f"bank_{r.lower()}"] = snap["bank"][ri]
        feats["dev_deck_remaining"] = snap["dev_deck_remaining"]
        # globalna liczba zagranych kart KAŻDEGO typu (suma po wszystkich graczach)
        for d in DEV_TYPES:
            feats[f"played_{d.lower()}_total"] = sum(
                ps.get(f"P{p}_PLAYED_{d}", 0) for p in range(4))
        # historia obserwowanego
        ...
        # target licznościowy (pomocniczy)
        for d in DEV_TYPES:
...
def play_one_game(game_id):
    ...
    random.seed(game_id)  # boty używają globalnego random - determinizm
    ...
    if not strong:
        return [], []  # brak silnego bota -> pomijamy grę (nie rozgrywamy)

    observed_index = rng.choice(strong)        # DOKŁADNIE JEDNA perspektywa
    observed_color = COLORS[observed_index]
    ...
    try:
        ...
        print(f"Error occurred while playing game {game_id}: {e}")
        return [], []
    ...
    for idx, snap in enumerate(snapper.snaps):
        if idx < len(recs) and snap["action_type"] == "BUY_DEVELOPMENT_CARD":
            snap["buy_type"] = recs[idx].action.value

    winning_color = game.winning_color()
    winner_type = table[COLORS.index(winning_color)] if winning_color else None
    table_str = "|".join(table)

    return build_perspective(snapper.snaps, observed_color, game_id, table_str,
                             table[observed_index], winner_type)
...
def generate(num_games, out_dir, workers, chunk_size=1000, start_id=0):
    ...
    print("=" * 64)
    print(f"URUCHOMIENIE GENERATORA: {num_games} gier "
          f"(ID {start_id}..{start_id + num_games - 1})")
    print("=" * 64)
    ...
    for chunk_idx in range(num_chunks):
        ...
        print(f"[{chunk_idx + 1}/{num_chunks}] Gry ID {chunk_start_id}..{chunk_end_id - 1}...")
        ...
        skipped += sum(1 for ts, _ in results if not ts)  # gry pominięte (brak silnego)
        ...
        if ts_rows:
            ...
            ts_path = os.path.join(out_dir,
                f"timesteps_{chunk_start_id:05d}_to_{chunk_end_id-1:05d}.parquet")
            card_path = os.path.join(out_dir,
                f"card_samples_{chunk_start_id:05d}_to_{chunk_end_id-1:05d}.parquet")
            ts_df.to_parquet(ts_path, index=False, compression="snappy")
            card_df.to_parquet(card_path, index=False, compression="snappy")
            print(f" -> {len(ts_df):,} krokow -> {os.path.basename(ts_path)}")
            del ts_df, card_df

        del results, ts_rows, card_rows
        gc.collect()

    dt = time.perf_counter() - t0
    print("=" * 64)
    print(f"ZAKONCZONO. Gry {start_id}..{start_id + num_games - 1}")
    print(f"Pominieto gier (brak silnego bota): {skipped}/{num_games}")
    print(f"timesteps: {total_ts_rows:,} | card_samples: {total_card_rows:,}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-id", type=int, default=0,
                        help="Poczatkowe ID gry (do wznawiania)")
